<template>
	<div class="returnImport">
		...
		<el-dialog title="上传提示" :visible.sync="uploadVisible" width="500px">
			<el-upload ref="uploadDom" action="/mgmt/inventory/return/config/excelBatchImport" :http-request="uploadCustom"
...
export default {
	...
	methods: {
		// 下载模板
		downloadTemplate() {
			exportExcel(`/mgmt/inventory/return/config/returnConfigTemplateDownLaod`);
			this.active = 1;
		},
		// 上传文件
		uploadFile() {
		...
		// 导出失败记录
		exportFailLog() {
			exportExcel(`/mgmt/inventory/return/config/exportFailList`);
		},
	},
};
</script>
