<template>
	<div class="gradeImport">
		...
		<el-dialog title="上传提示" :visible.sync="uploadVisible" width="500px">
			<el-upload ref="uploadDom" action="/mgmt/store/importBatchAdjustLevel" :http-request="uploadCustom">
...
export default {
	...
	methods: {
		// 下载模板
		downloadTemplate() {
			exportExcel(`/mgmt/store/exportBatchAdjustLevelTemplate`);
			this.active = 1;
		},
		// 上传文件
		uploadFile() {
		...
		// 导出失败记录
		exportFailLog() {
			exportExcel(`/mgmt/store/exportBatchAdjustLevelFailRecord`);
		},
	},
};
</script>
