<template>
	<div class="unknown-flow-deal">
		<guideBar>
			<el-button v-permission="'unknown_flow_deal_export'" type="primary" @click="batchExport">导出EXCEL</el-button>
		</guideBar>
		<el-card>
				...
				<el-tab-pane label="待处理" name="pending">
					...
					>
						<template slot-scope="scope">
							<el-button v-permission="'unknown_flow_deal_detail'" type="text" @click="detail(scope.data)">
								详情
							</el-button>
							<el-button v-permission="'unknown_flow_deal_process'" type="text" @click="dealWith(scope.data)">
								处理
							</el-button>
						</template>
					</tableList>
...
export default {
	...
	data() {
		return {
			...
			paymentAdjustMap: [], // 款项类型和调整原因规则映射
	...
	methods: {
		// 导出EXCEL
		batchExport() {
			exportExcel('/mgmt/inventory/remit/exportUnknownRemitExcel', this.loadDataPost);
		},
		// 搜索
		handleSearch() {
		...
		dealWith(row) {
			this.bankRemitProps = {
				...
				show: true,
				type: 2, // 处理
				storeCodeSearch: true, // 是否支持服务中心编号搜索
			};
		},
		// 处理 -- 取消
		dealCancel() {
		...
		async loadPaymentAdjustMap() {
			...
			if (res && res.data) {
				...
				this.bankRemitProps.sourceTypeList = [...res.data.slice(4, 6)];
				res.data.map((item) => {
