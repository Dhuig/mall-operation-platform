<template>
	<div class="charging-detail">
		<guideBar>
			<el-button type="primary" @click="goToBill" v-if="showEdit">修改押货单</el-button>
		</guideBar>
		<el-form :inline="true" ref="formRef" label-width="180px" :model="ruleForm" :rules="rules" label-suffix=":">
			...
			<el-card class="box-card">
				...
				<!-- <el-table :data="detailList" style="width: 100%;" border>
					...
					<el-table-column prop="productBoxNum" label="装箱数量"></el-table-column>
					<el-table-column prop="productMortgagePrice" label="单价"></el-table-column>
					<el-table-column prop="productPv" label="PV"></el-table-column>
					<el-table-column prop="productNum" label="押货数"></el-table-column>
					<el-table-column prop="productDeliveredNum" label="已发货数"></el-table-column>
				</el-table>-->
			</el-card>
			<!-- 发货记录 -->
			<el-card class="box-card" v-if="[3, 4].includes(detailInfo.orderVo.orderStatus)">
				<div slot="header" class="clearfix">
					<span>发货记录</span>
				</div>
				<div class="collpase-box">
					<el-collapse accordion v-model="activeName" v-if="list.length > 0">
						<el-collapse-item v-for="(item, index) in list" :key="index" :name="index + 1">
							...
							<div style="margin: 25px 0;">
								<tableList
									:dataSource="item.deliverItemList"
									:columns="DELIVERY_RECORD_COLUMNS"
									:options="optionsReviewed"
									border
								></tableList>
				...
				<el-row class="min-width">
					<el-col :span="24">
						<el-form-item label="审核备注" prop="auditRemarks">
							<el-input type="textarea" placeholder="请输入" v-model="ruleForm.auditRemarks" class="self-width" />
						</el-form-item>
					</el-col>
				</el-row>
			</el-card>
