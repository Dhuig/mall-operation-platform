		...
		<el-form :inline="true" ref="formRef" label-width="180px" :model="ruleForm" :rules="rules" label-suffix=":">
			...
			<el-card class="box-card">
				...
				<el-table :data="detailList" style="width: 100%;" border>
					...
					<el-table-column prop="productSecCode" label="二级编码" min-width="150"></el-table-column>
					<el-table-column prop="productSecName" label="二级产品名称" min-width="150"></el-table-column>
					<el-table-column prop="productPacking" label="规格" min-width="120"></el-table-column>
					<el-table-column prop="productUnit" label="单位" min-width="80"></el-table-column>
					<el-table-column prop="productBoxNum" label="装箱数量" min-width="80"></el-table-column>
					<el-table-column prop="productMortgagePrice" label="押货价" min-width="100">
						<template slot-scope="scope">
							<span>¥{{ detailList[scope.$index].productMortgagePrice }}</span>
						</template>
					</el-table-column>
					<el-table-column prop="productPv" label="PV"></el-table-column>
					<el-table-column prop="productInventoryNum" label="库存数量"></el-table-column>
					<!-- <el-table-column prop="canMortgageNum" label="最高可押货量" min-width="120"></el-table-column> -->
					...
					<el-table-column prop="productDeliveredNum" label="已发货数" min-width="80"></el-table-column>
				</el-table>
			...
			<actionBar>
				<el-button type="primary" :loading="submitLoading" @click="handleSave('formRef')">确认修改</el-button>
...
export default {
	...
	methods: {
		...
		productNumChange(info, index) {
			if (info.maxProductNum < info.productDeliveredNum) {
		...
		handleSave(formName) {
			...
			this.$refs[formName].validate(async valid => {
				if (valid) {
					...
					updateInvtMortgageOrderVO.orderRemarks = `定制品单 ${this.orderRemarks}`;
			...
			if (res && res.code === 200) {
				if (res.data && res.data.length !== 0) {
					let allCode = [],
						allNum = [];
					res.data.map(item => {
						allCode.push(item.productSecCode);
					...
					let cont = `${this.detailInfo.orderVo.storeCode}服务中心 ${allCode.join(
						...
						'、'
					)}，目前退货数量已超出当前库存，原则上不予欠货停发。请确认是否继续执行？`;
		...
		async updateMortgageOrder(params) {
			...
			if (res && res.code === 200) {
				...
				this.$closeSelf();
				this.$go('customized_detail', { title: '定制品押货单详情', id: this.orderId, type: 'detail' });
			}
			if (res && res.code == 0) {
				this.$message({
					type: 'error',
