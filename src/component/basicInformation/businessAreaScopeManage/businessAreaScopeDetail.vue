		...
		<el-card class="box-card">
			<div class="my">
				...
				<el-button
					type="primary"
					v-if="state === 'edit'"
					v-permission="'business_area_scope_detail_add'"
					@click="addProvinces"
				>
					添加省份
				</el-button>
						...
						<div class="table-content" v-for="(child, idx) in item.childrenList" :key="idx">
							...
							<div class="region">
								<div v-for="(cd, i) in child.childrenList" :key="i" class="mr-16">
									<span class="area_span">{{ cd.name }}</span>
									<!-- <el-checkbox
										:label="cd.name"
										v-model="cd.check"
			...
			<div class="btn-footer">
				<el-button
					type="primary"
					v-if="state === 'edit'"
					v-permission="'business_area_scope_detail_save'"
					@click="onSubmit"
				>
					保存选择
				</el-button>
...
export default {
	...
	methods: {
			...
			this.$refs[formName].validate(async valid => {
				...
				if (valid) {
					...
					const res = await this.$fetch({
						...
						data: { bindCodeList: this.bindCodeList, untieCodeList: this.untieCodeList, companyId: id, regionLevel: 1 },
					});
					if (res && res.code === 200) {
		...
		handleCheckAllChange(val, check) {
			...
			console.log(val.childrenList.filter(x => !x.disabled));
			...
			if (val.childrenList && val.childrenList.length > 0) {
				...
				this.findChildren(
					val.childrenList.filter(x => !x.disabled),
					check
				);
			} else {
				// 无下级处理本级
		...
		async onSubmit() {
			...
			const res = await this.$fetch({
				method: 'post',
				url: '/mgmt/sys/company/business/region/save',
				data: { bindCodeList, companyId: id, untieCodeList, regionLevel: 2 },
			});
			if (res && res.code === 200) {
		...
		async init() {
			...
			if (res && res.code == 200 && res.data) {
				...
				this.data = res.data.map(item => {
					this.provinceList.push(item.regionCode);
					return {
						name: item.regionName,
						code: item.regionCode,
						disabled: this.state === 'detail' ? true : item.other,
						check: item.self,
						childrenList:
							item.childList &&
							...
							item.childList.map(a => {
								return {
									...
									name: a.regionName,
									code: a.regionCode,
									disabled: this.state === 'detail' ? true : a.other,
									check: a.self,
									childrenList:
										a.childList &&
										...
										a.childList.map(b => {
											return {
												...
												name: b.regionName,
												code: b.regionCode,
												// disabled: this.state === 'detail' ? true : b.other,
												// check: b.self,
