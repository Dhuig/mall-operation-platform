	...
	<div class="userManagement">
		<guideBar>
			<div v-if="activeTab == 'product'" class="marginR10">
				<el-button v-permission="'inventory_query_history_exportPDF'" type="primary" @click="exportPDF">
					导出PDF
				</el-button>
				<el-button v-permission="'inventory_query_history_print'" type="primary" :loading="printLoading" @click="print">
					打印
				</el-button>
			</div>
			<el-button
				:loading="exportLoading"
				v-permission="'inventory_query_history_export'"
				type="primary"
				@click="batchExport"
			>
				批量导出
		...
		<el-card>
			...
			>
				<template slot-scope="row" v-if="activeTab == 'product'">
					<el-button type="text" @click="handleBtnsGruop(row.data)">增减明细</el-button>
				</template>
			</tableList>
		</el-card>
		<!-- 打印表格 -->
...
import {
	REPERTORY_MONTH_HISTORY_PRODUCT_COLUMNS,
	REPERTORY_MONTH_HISTORY_SERVICE_COLUMNS,
	REPERTORY_NARNAL_HISTORY_PRODUCT_SEARCH,
	REPERTORY_NARNAL_HISTORY_SERVICE_SEARCH,
...
export default {
	...
	computed: {
		config: vm =>
			vm.activeTab == 'product' ? REPERTORY_NARNAL_HISTORY_PRODUCT_SEARCH : REPERTORY_NARNAL_HISTORY_SERVICE_SEARCH,
		columns: vm =>
			vm.activeTab == 'product' ? REPERTORY_MONTH_HISTORY_PRODUCT_COLUMNS : REPERTORY_MONTH_HISTORY_SERVICE_COLUMNS,
		slotcontent: vm => vm.activeTab == 'product',
		height: vm => (vm.activeTab == 'product' ? 500 : 460),
		// 搜索参数
		searchFields() {
			const { searchConfig } = this.tabPanes[this.activeTab];
			console.log(searchConfig);
			let params = {
				...searchConfig,
				month: searchConfig.month
					? formatDate(searchConfig.month, 'month').replace('-', '')
					: formatDate(new Date(), 'month').replace('-', ''),
			};
			return params;
		},
	},
	...
	methods: {
		...
		exportPDF() {
			...
			exportExcel('/mgmt/inventory/bill/history-export-pdf', params);
		},
		// 打印
		async print() {
			...
			const res = await this.$fetch(`/mgmt/inventory/bill/history-print?${qs.stringify(params)}`);
		...
		async getCompanyCode() {
			...
			if (res?.code === 200) {
				...
				this.REPERTORY_NARNAL_HISTORY_PRODUCT_SEARCH.form[0].options = code;
				this.REPERTORY_NARNAL_HISTORY_SERVICE_SEARCH.form[0].options = code;
			} else {
		...
		async getSummaries() {
			...
			const url = this.activeTab == 'product' ? '/mgmt/inventory/bill/total?' : '/mgmt/inventory/bill/store/total?';
		...
		async loadData() {
			...
			const url = this.activeTab == 'product' ? '/mgmt/inventory/bill?' : '/mgmt/inventory/bill/store?';
			const res = await this.$fetch({ url: `${url}${qs.stringify(params)}`, timeout: 60000 * 1 });
			if (res) {
