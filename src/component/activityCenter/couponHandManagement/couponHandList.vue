	...
	<div class="platform-activeCenter_couponHandList-wrap">
		<guideBar>
			<el-button
				...
				v-permission="'coupon_handList_batch_export'"
				type="primary"
				@click="loadData(1)"
			>
				导出
		...
		<el-card>
			<searchPanel ref="searchPanel" v-model="searchModel" :config="config" @search="handleSearch" />
		</el-card>
		<el-card>
...
export default {
	...
	data() {
		return {
			...
			config: COUPON_SEARCH_LIST_CONFIG,
			tabPane: TAB_PANE,
	...
	created() {
		this.$listen('CONPONHANDLIST', this.handleSearch);
	},
	methods: {
		stopClose() {
		...
		loadMore() {
			let { currentPage, pageSize, total } = this.tabPaneObj[this.activeName].pagination;
			if (currentPage * pageSize >= total) return this.$message.warning('没有更多数据');
			this.tabPaneObj[this.activeName].pagination.currentPage++;
			this.loadData();
		},
		async loadData(status) {
			...
			} = this.tabPaneObj[this.activeName];
			if (status) {
				exportExcel(`/mgmt/prmt/couponGrant/exportCouponGrantList`, {
					...searchData,
					state: this.state,
					ids: this.ids,
					grantWay: 1,
			...
			const res = await this.$fetch({
				...
				params: { ...searchData, pageNum, pageSize, state: this.state, grantWay: 1 },
			});
			if (res && res.code == 200) {
				const { list, total } = res.data;
				this.tabPaneObj[this.activeName].pagination.total = total || 0;
				// 执行concat拼接数据达到加载;
				// this.tabPaneObj[this.activeName].data = this.tabPaneObj[this.activeName].data.concat(list);
		...
		detail({ id }) {
			this.$go('coupon_handOf_Detail', { id });
		},
		edit({ id }) {
			this.$go('create_handWork', { edit: id });
		},
		submitClick({ id }) {
