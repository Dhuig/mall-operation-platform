	...
	<div>
					...
					<div class="head-class-table">
						<div class="head-table">
							...
							<span class="head-right">
								...
								<span class="left" v-if="pageData.limitType == 2">
									每位顾客可以购买每样产品{{ pageData.limitNumber }}件
								</span>
						...
						<el-button
							class="top-btn-dc"
							type="primary"
							v-permission="'spike_activity_customer_export'"
		...
		<el-dialog title="手动添加" v-drag :visible.sync="dialogVisible" width="50%" :before-close="handleClose">
			<div class="search-div">
				...
				<el-table
					v-if="addList.length > 0 && pageData.limitType == 4"
					:data="productList"
					border
					style="width: 100%"
					:loading="false"
...
import { CUSTOMER_CONFIG, LINMIT_MUMBER, CUSTOMER_RECORD, RECORD_COLUMNS } from '../config';
...
export default {
	...
	methods: {
		...
		async customerData(status) {
			...
			if (status) {
				if (this.pageData.limitType == 4) {
					exportExcel(`/mgmt/prmt/exportPromotionMember`, {
						promotionId: id,
						...this.searchData,
						limitNumber: '',
						limitType: 4,
		...
		handleSuccess(file) {
			let { limitNumber, limitType } = this.pageData;
			let { id: promotionId } = this.$route.query;
			fromDataUpload.call(
				this,
				{
					url: `/mgmt/prmt/importUserList`,
					formObject: { promotionId, limitNumber, promotionType: 1, limitType, file: file.file },
				},
				res => {
					if (res.code === 200) {
						this.$message.success(res.message);
		...
		async save() {
			...
			let params = { limitNumber, limitType, cardNo, mobile, realName, promotionId, productLists: this.productList };
			const res = await this.$fetch({
				method: 'POST',
