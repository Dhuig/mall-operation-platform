		...
		<el-card class="card-gap">
			...
			<div class="dr">
				<span class="dr-btns">
					...
					<el-button type="primary" class="top-btn-dr" @click="getExcel">下载模板</el-button>
						...
						<!-- <template slot-scope="{ row, $index }">
							<div>
								<el-select
									...
									filterable
									placeholder="输入产品名称"
									:remote-method="productNameMethod"
									:loading="optionsLoading"
									reserve-keyword
									remote
									@change="selChange($event, $index)"
									@focus="productNameoptions = []"
								>
									<el-option
...
export default {
	...
	data() {
		return {
			...
			importVisible: false,
			dialogVisible: false,
			uploadData: { promotionId: null, promotionType: 1 },
	...
	watch: {
		promotion: {
			handler(val) {
				...
				this.uploadData = { promotionId: query.id, promotionType: 1, limitNumber };
	...
	methods: {
		...
		handleSuccess(res) {
			...
			// 	this.productDtosArr = res.data.concat(this.productDtosArr);
			// 	let arr = new Set(
			// 		this.productDtosArr.map(e => {
			// 			if (e.serialNo !== '') {
			// 				return e.serialNo;
			// 			}
			// 		})
			...
			// 	let qc = [];
			// 	arr.forEach(e => {
			// 		for (let index = 0; index < this.productDtosArr.length; index++) {
			// 			if (e && e === this.productDtosArr[index].serialNo) {
			// 				qc.push(this.productDtosArr[index]);
			// 				break;
			// 			}
			// 		}
			// 	});
			// 	this.productDtosArr = qc;
			// } else {
			// 	this.$message.error(res.message);
			// }
		},
		...
		async getProduct(couponNumber = '') {
			const res = await this.$fetch(`/mgmt/prmt/getProductListByCodeOrName?productCode=${couponNumber}&type=1`);
			if (res && res.code == 200) {
				let { data } = res;
				this.temporary = data;
				this.product = data[0] || {};
