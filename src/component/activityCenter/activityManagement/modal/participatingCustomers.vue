	...
	<div>
		<el-card class="card-gap">
			<div class="head-text">
				...
				<span class="text-two">
					限购数量:
					<span class="left" v-if="promotion.configDto.limitType == 1">不限量</span>
					<span class="left" v-if="promotion.configDto.limitType == 2">
						每位顾客可以购买每样产品{{ promotion.configDto.limitNumber }}件
					</span>
					<span class="left" v-if="promotion.configDto.limitType == 3">
						每位顾客可以购买所有活动产品总共{{ promotion.configDto.limitNumber }}件
					</span>
					<span class="left" v-if="promotion.configDto.limitType == 4">按需导入</span>
			...
			<el-form :model="promotion.configDto" :rules="rules" ref="ruleForm" label-width="140px">
				<el-form-item label="顾客限制：" prop="limitCustomer">
					<template v-if="promotion.configDto.limitType == 4">
						<div>
							<el-radio v-model="promotion.configDto.limitCustomer" :label="4">自由导入</el-radio>
						</div>
					</template>
					<template v-else>
						<el-radio
		...
		<el-card class="card-gap">
			<div class="all-content" v-if="promotion.configDto.limitCustomer == 1 && promotion.configDto.limitType !== 4">
				所有顾客都可以参与活动
			</div>
			...
			<el-form :rules="rules" ref="ruleForm" label-width="140px" v-show="promotion.configDto.limitCustomer == '2'">
				...
				<el-form-item label="仅当月新开卡：" required>
			...
			<el-form :inline="true" label-width="140px" v-show="promotion.configDto.limitCustomer == '2'">
				<div class="head-text text-one" v-if="promotion.configDto.limitCustomer == '2'">导入不可购顾客名单</div>
				<el-form-item label="搜索已导入顾客：" prop="cardNo">
					<el-input v-model="purchaseNot.cardNo" clearable placeholder="输入会员卡号"></el-input>
				</el-form-item>
				<el-form-item label>
					<el-button type="primary" @click="purchaseNotserach">搜 索</el-button>
				</el-form-item>
				<el-form-item label>
					<el-button type="primary" plain @click="delVisible = true">清空导入列表</el-button>
				</el-form-item>
				<el-form-item label>
					<el-button type="primary" plain @click="importDisableMemberTemplate">下载模板</el-button>
				</el-form-item>
				<el-form-item label>
					<el-upload
						class="upload-demo"
						:on-error="onError"
						:action="url + '/mgmt/prmt/importDisableList'"
						name="file"
						:data="uploadData"
			...
			<div class="table-box">
				<el-col v-show="promotion.configDto.limitCustomer == '2'">
					<el-table :data="purchaseNot.data" style="width: 100%">
						...
						<el-table-column prop="zip" label="操作">
							<template slot-scope="scope">
								<div>
									<span class="btns" @click="purchaseDel(scope.row)">删除</span>
								</div>
							</template>
						</el-table-column>
					</el-table>
					<el-pagination
						background
						class="pagination"
						:total="purchaseNot.total"
						:current-page.sync="purchaseNot.pagination.currentPage"
						:page-size="purchaseNot.pagination.pageSize"
						@current-change="purchaseNotPage"
						@size-change="purchaseNotSize"
			...
			<el-form
				:inline="true"
				label-width="140px"
				v-show="promotion.configDto.limitCustomer == 4 || promotion.configDto.limitType === 4"
			>
				<div class="head-text text-one" v-if="promotion.configDto.limitCustomer == 4">自由导入</div>
				<el-form-item label="搜索已导入顾客：" prop="cardNo">
					<el-input v-model="identityList.cardNo" clearable placeholder="输入会员卡号"></el-input>
				</el-form-item>
				<el-form-item label>
			...
			<div class="table-box">
				<el-col v-show="promotion.configDto.limitCustomer == 4 || promotion.configDto.limitType === 4">
					<el-table :data="identityList.data" style="width: 100%">
						<el-table-column prop="cardNo" label="会员卡号"></el-table-column>
						<el-table-column prop="realName" label="会员姓名"></el-table-column>
						<el-table-column prop="mobile" label="注册手机号"></el-table-column>
						<el-table-column
							prop="serialNo"
							label="产品编码"
							v-if="promotion.configDto.limitType === 4"
						></el-table-column>
						<el-table-column
							prop="totalCount"
							label="可购数量"
		...
		<el-dialog title="手动添加" v-drag :visible.sync="dialogVisible" width="50%" :before-close="handleClose">
			<div class="search-div">
				...
				<el-table
					v-if="addList.length > 0 && promotion.configDto.limitType == 4"
					:data="productList"
					border
					style="width: 100%"
					:loading="false"
...
import { PARTICIPATING_CUSTOMER_COLUMNS, LIMIT, ADD_COLUMNS, LINMIT_MUMBER } from '../config.js';
...
export default {
	...
	watch: {
		promotion: {
			handler() {
				...
				if (this.promotion.configDto.limitType == 4) this.promotion.configDto.limitCustomer = 4;
				if (this.promotion.configDto.limitCustomer === 4) this.identityData();
				if (this.promotion.configDto.limitCustomer == 2) this.getDisableUserPage();
			},
			immediate: true,
			deep: true,
		},
	...
	created() {
		...
		let { limitNumber, limitType } = this.promotion.configDto;
		this.uploadData =
			query && query.id
				? { promotionId: query.id, limitNumber, limitType, promotionType: 1 }
				: { limitNumber, limitType, promotionType: 1 };
	...
	methods: {
		...
		async getDisableUserPage() {
			let { query } = this.$route;
			let { cardNo, promotionId } = this.purchaseNot;
		...
		async deleteImportDisableMember() {
			let { query } = this.$route;
			const res = await this.$fetch({
				method: 'POST',
		...
		delImportData() {
			if (this.promotion.configDto.limitCustomer == '2') {
				this.deleteImportDisableMember();
			} else {
		...
		handleSuccess(file) {
			...
			if (limitCustomer == '2') {
				this.purchaseNot.loading = false;
			} else {
			...
			fromDataUpload.call(
				this,
				{
					url: `/mgmt/prmt/${limitCustomer == '2' ? 'importDisableList' : 'importUserList'}`,
		...
		save() {
			...
			if (this.promotion.configDto.limitCustomer == '2') {
				this.addDisableUser({ cardNo, id, mobile, promotionId, realName });
			} else {
				this.addUser(params);
			}
		},
		async addDisableUser(params) {
			const res = await this.$fetch({
				method: 'POST',
		...
		serachList() {
			this.identityList.pagination.currentPage = 1;
			this.identityData();
		},
	},
};
