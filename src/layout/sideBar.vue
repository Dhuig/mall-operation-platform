<template>
	<div
		:class="['platform-sidebar', !expand && 'sidebar-shrink']"
		ref="dom"
		@mouseenter="cancelHide"
		@mouseleave="hideMenuDelay"
	>
		<div :class="['sidebar-searchbar', keyword.length && 'searchbar-focus']" v-show="expand">
			<input
				...
				@focus="clearSubMenu"
			/>
			<i class="el-icon-close sidebar-clear-button" @click="clearKeyword" v-show="keyword.length" />
		...
		<ul class="top-menu-list">
			<li
				...
				:key="idx"
				:class="['sidebar-item', activeMenu === idx && 'active-menu', !expand && 'sidebar-item-shrink']"
				@click="goTo(item)"
			...
			>
				...
				<i class="el-icon-caret-right" v-show="item.children && item.children.length && activeMenu === idx" />
			</li>
		</ul>
		<!-- 二级菜单 -->
		<transition name="el-fade-in">
			<ul :class="['sub-menu-list', !expand && 'sub-menu-list-shrink']" v-show="showSubMenu">
				<li
					v-for="(subItem, subIdx) in displaySubMenu"
					:key="subIdx"
					:class="['sidebar-item sidebar-sub-item', activeSubMenu === subIdx && 'active-menu']"
...
export default {
	...
	methods: {
		...
		// 延迟隐藏菜单
		hideMenuDelay() {
			this.t = setTimeout(this.clearSubMenu, 400);
		},
		// 跳转
		goTo(item) {
			if (!item || !item.href || (item.children && item.children.length)) return;
			this.clearSubMenu();
			// if (pathFormat(item.href) === pathFormat(this.$route.path)) return;
